...
figure
imshow(I_rgb);title('Imágen Original');
...
armado_img = zeros(M_final,N_final,p,'double'); %Para poder ver el proceso
armado = mat2cell(armado_img, repmat(m_pieza,1,4), repmat(n_pieza,1,4), p);

% Mi pieza semilla va a ser la (1,1)
armado{1,1} = piezas{1,1};

I_proceso = cell2mat(armado); % Condicin inicial
figure
imshow(lab2rgb(I_proceso)); title('Pieza semilla');
...
for i=1:4
    for j=1:4
        ...
        I_proceso = cell2mat(armado);
        figure
        imshow(lab2rgb(I_proceso)); title('Pasos')

    end
end

%% Paso de celdas a una imagen nuevamente y grafico

I_rearmada = cell2mat(armado);

figure
imshow(lab2rgb(I_rearmada));title('Imagen armada');
